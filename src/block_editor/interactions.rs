...
impl BlockEditor {
    ...
    pub fn mouse_double_click(&mut self, mouse: &MouseEvent, source: &str, ctx: &mut EventCtx){
        let x ;
        let y = (mouse.pos.y / FONT_HEIGHT) as usize;//Mouse y positition will be the same 
        // request keyboard focus if not already focused
        if !ctx.is_focused() {
            ctx.request_focus();
        }
